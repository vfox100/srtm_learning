...
def find_intersection(
    ...
    altitude = geo_data.get_elevation(center_latitude, center_longitude)
    lat_max, lat_min, lon_max, lon_min = bound(
        center_latitude, center_longitude, radius
    )
    assert altitude is not None
    starting_point = (center_longitude, center_latitude, altitude)
    ...
    GD1 = interpolate.griddata(
        (x1, y1), dem_interpolated.ravel(), (xx, yy), method="cubic"
    )
    ...
    if verbose:
        ...
        scale_lon = ((lon_max - lon_min) * 111132 * math.cos(center_latitude)) / SIZE[
            1
        ...
        sphere = pv.Sphere(
            radius=1,
            center=[math.floor(512 / 2), math.floor(512 / 2), altitude / scale_lat],
        )
    ...
    return get_intersection(starting_point, heading)
